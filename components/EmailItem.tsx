  ...
  return (
    <div
      className={`px-4 py-4 cursor-pointer transition-colors border-b border-[#2a2a2a] ${
        isSelected ? 'bg-[#252525]' : 'hover:bg-[#1e1e1e]'
      }`}
    >
      {/* Row 1: Priority dot + Sender + Time */}
      <div className="flex items-center justify-between mb-1.5">
        <div className="flex items-center gap-2.5 min-w-0">
          {priority && <div className={`w-[7px] h-[7px] rounded-full ${priority.dot} shrink-0`} />}
          <span
      ...
      {/* Row 3: Preview */}
      <p className="text-[12px] text-zinc-600 line-clamp-2 leading-relaxed mb-2">{previewText}</p>

      {/* Row 4: Priority pill */}
      {priority && (
        <span
          className={`inline-block text-[10px] font-bold tracking-wider px-2 py-[3px] rounded ${priority.bg} ${priority.text} font-mono`}
        >
          {priority.label}
        </span>
      )}
