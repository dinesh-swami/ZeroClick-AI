    ...
    if (to && to.includes(',')) {
      return to
        ...
        .map((t, i) => (
          <div
            key={i}
            className="flex items-center gap-1.5 bg-[#2a2a2a] border border-[#333] rounded-full px-3 py-1 mr-2 text-[13px] text-zinc-300"
          >
            <div
              className={`w-1.5 h-1.5 rounded-full ${i % 2 === 0 ? 'bg-emerald-500' : 'bg-blue-500'}`}
            />
            {t}
            <button
      ...
      >
        ...
        <div className="p-5 flex items-center justify-between shrink-0 border-b border-[#222]">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-[15px] font-semibold text-white tracking-tight leading-none mb-1.5">
                Compose message
              </h2>
        ...
        <div className="p-5 flex flex-col gap-6">
          ...
          <div>
            ...
            <div
              className="flex items-center flex-wrap gap-y-2 bg-[#151515] border border-[#222] rounded-xl px-3 py-2 min-h-[44px] cursor-text"
              onClick={() => document.getElementById('compose-to')?.focus()}
            >
              {renderChips()}
              <input
                id="compose-to"
