...
export async function detectSmartChips(
  ...
  try {
    const { object } = await generateObject({
      ...
      system:
        'You are an AI assistant that extracts calendar event details and bug/feature reports from emails. If there is no explicit meeting proposed, set hasMeeting to false. If there is no bug/feature reported, set hasBugOrFeature to false.',
      ...
      schema: z.object({
        hasMeeting: z
          .boolean()
          .describe('True if the email proposes or confirms a meeting, call, or event.'),
        title: z
          .string()
          .optional()
          .describe('A suitable title for the calendar event or linear issue.'),
        suggestedTime: z
          .string()
          .optional()
          .describe('The proposed date and time mentioned in the email, if any.'),
        durationMinutes: z
          .number()
          .optional()
          .describe(
            'Estimated duration in minutes, default to 30 if not specified but a meeting exists.'
          ),
        attendees: z
          .array(z.string())
          .optional()
          .describe('List of email addresses or names that should attend.'),
        hasBugOrFeature: z
          .boolean()
          .describe('True if the email reports a software bug, issue, or requests a new feature.'),
      }),
    });
...
export async function extractMeetingActionItems(threadContent: string) {
  ...
  try {
    const { object } = await generateObject({
      ...
      system:
        'You are an executive assistant analyzing a post-meeting email thread to extract action items for a follow-up email.',
      ...
      schema: z.object({
        actionItems: z.array(
          z.object({
            ...
            owner: z
              .string()
              .describe('The person responsible for the action. Use "Unknown" if not specified.'),
...
export async function collapseThreadDecisionLog(threadContent: string) {
  ...
  try {
    const { object } = await generateObject({
      model: openrouter('anthropic/claude-3.5-sonnet'),
      system:
        'You are an intelligent summarization engine that distills long email threads into clear, structured decision logs for executives.',
      prompt: `Analyze the following email thread and collapse it into a structured decision log.\n\nThread Context:\n${content}`,
      schema: z.object({
        decisionsMade: z
          .array(z.string())
          .describe('List of clear decisions that were agreed upon in the thread.'),
        openQuestions: z
          .array(z.string())
          .describe('List of questions that remain unanswered or require further discussion.'),
        actionItems: z
          .array(
            z.object({
              task: z.string(),
        ...
        tlDr: z
          .string()
          .describe('A very brief 2-3 sentence executive summary of the entire thread.'),
      }),
    });
