...
export class CalendarService {
  static async getEvents(userId: string, timeMin?: string, timeMax?: string) {
    ...
    try {
      ...
      await t.googlecalendar.api.events
        .getMany({ calendarId: 'primary', maxResults: 1 })
        .catch(() => {});
      ...
      if (calListData.error) {
        console.error(
          '[CalendarService] Calendar list API error:',
          JSON.stringify(calListData.error)
        );
        return [];
      }
      ...
      const eventPromises = activeCalendars.map(async (cal: any) => {
        try {
          ...
          console.error(
            `[CalendarService] Failed to fetch events for calendar ${cal.id}:`,
            err.message
          );
          return [];
        }
