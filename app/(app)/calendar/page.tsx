...
export default function CalendarPage() {
  ...
  const hasConflicts =
    freeBusyData && Object.values(freeBusyData).some((cal: any) => cal.busy && cal.busy.length > 0);
        ...
        <div className="h-24 flex items-center justify-between px-8 shrink-0 border-b border-[#222]">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2">
              <button
                onClick={handlePrev}
                className="p-2 bg-[#151515] border border-[#222] hover:bg-[#222] text-zinc-400 hover:text-white rounded-lg transition-colors"
              >
                <ChevronLeft className="w-4 h-4" />
              </button>
              <button
                onClick={handleToday}
                className="px-4 py-1.5 bg-[#151515] border border-[#222] hover:bg-[#222] text-[11px] font-bold text-zinc-400 hover:text-white rounded-lg transition-colors uppercase tracking-wider"
              >
                Today
              </button>
              <button
            ...
            <button
              ...
              className="ml-2 flex items-center gap-2 bg-zinc-100 hover:bg-white text-black px-4 py-2 rounded-lg text-[13px] font-bold transition-colors"
            >
        ...
        {view === 'month' ? (
          <div className="flex-1 flex flex-col overflow-hidden bg-[#0a0a0a]">
            ...
            <div className="grid grid-cols-7 border-b border-[#222] shrink-0">
              {['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map((day) => (
                <div
                  key={day}
                  className="py-2 text-center text-[11px] font-bold text-zinc-500 uppercase tracking-widest border-r border-[#222] last:border-r-0"
                >
                  {day}
                </div>
              ))}
            ...
            <div className="flex-1 grid grid-cols-7 auto-rows-[1fr] overflow-hidden">
              {days.map((day, i) => {
                ...
                return (
                  <div
                    key={i}
                    className={`border-r border-b border-[#222] flex flex-col p-1 overflow-hidden cursor-pointer hover:bg-white/5 transition-colors ${isCurrentMonth ? 'bg-transparent' : 'bg-[#111] opacity-50'}`}
                    onClick={() => {
                      setView('day');
                      setCurrentDate(day);
                    }}
                  >
                    <div
                      className={`text-[12px] font-bold p-1 w-6 h-6 flex items-center justify-center rounded-full mb-1 ${isToday(day) ? 'bg-white text-black' : 'text-zinc-400'}`}
                    >
                      {format(day, 'd')}
                    </div>
                    <div className="flex-1 overflow-y-auto flex flex-col gap-1 hide-scrollbar">
                      {dayEvents.map((event: any, idx: number) => {
                        const colorIdx = event.id
                          ? event.id.charCodeAt(0) % bgColors.length
                          : idx % bgColors.length;
                        return (
                          <div
                            key={event.id}
          ...
          <div className="flex-1 flex flex-col overflow-hidden bg-[#0a0a0a]">
              ...
              <div className={`flex-1 grid ${view === 'day' ? 'grid-cols-1' : 'grid-cols-7'}`}>
                {days.map((day, i) => {
                  return (
                    <div
                      key={i}
                      className="flex flex-col items-center justify-center py-3 border-r border-[#222] last:border-r-0"
                    >
                      <div className="text-[11px] font-bold text-zinc-500 uppercase tracking-widest mb-1">
                        {format(day, 'EEE')}
                      </div>
                      <div
                        className={`text-[18px] font-bold w-9 h-9 flex items-center justify-center rounded-full ${isToday(day) ? 'bg-white text-black' : 'text-zinc-200'}`}
                      >
                        {format(day, 'd')}
                      </div>
                    </div>
            ...
            <div className="flex-1 overflow-y-auto flex" ref={scrollContainerRef}>
              ...
              {/* Grid Columns */}
              <div
                className={`flex-1 grid ${view === 'day' ? 'grid-cols-1' : 'grid-cols-7'} relative`}
              >
                  ...
                  return (
                    <div key={i} className="relative border-r border-[#222] last:border-r-0">
                      ...
                      {dayEvents.map((event: any, idx: number) => {
                        ...
                        const c =
                          event.colorId && googleColors[event.colorId]
                            ? googleColors[event.colorId]
                            : {
                                bg: bgColors[colorIdx],
                                border: borderColors[colorIdx],
                                text: textColors[colorIdx],
                              };
