    ...
    <div className="flex flex-row h-full w-full bg-white text-zinc-900 overflow-hidden font-sans">
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="h-24 flex items-center justify-between px-8 shrink-0">
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="flex items-center gap-2">
              <button
                onClick={handlePrev}
                className="w-9 h-9 flex items-center justify-center bg-zinc-100 hover:bg-zinc-200 text-zinc-700 rounded-xl transition-colors"
              >
                <ChevronLeft className="w-4 h-4" />
              </button>
              <button
                onClick={handleToday}
                className="px-5 h-9 bg-zinc-100 hover:bg-zinc-200 text-[13px] font-semibold text-zinc-700 rounded-xl transition-colors"
              >
                Today
              </button>
              <button
            ...
            <button
              ...
              className="ml-2 flex items-center gap-2 bg-zinc-900 hover:bg-zinc-800 text-white px-5 h-9 rounded-xl text-[13px] font-semibold transition-colors shadow-sm"
            >
            ...
            {days.map((day, i) => {
              ...
              return (
                <div key={i} className="flex flex-col items-center justify-center py-4">
                  {isActive ? (
                    <div
                      className="bg-zinc-900 rounded-2xl flex flex-col items-center justify-center py-3 w-[100px] shadow-lg transform transition-transform hover:scale-105 cursor-pointer"
                      onClick={() => {
                        setView('day');
                        setCurrentDate(day);
                      }}
                    >
                      <div className="text-[12px] font-medium text-zinc-400 capitalize mb-0.5">
                        {format(day, 'EEEE')}
                      </div>
                    ...
                    <div
                      className="bg-zinc-50 rounded-2xl flex flex-col items-center justify-center py-3 w-[100px] hover:bg-zinc-100 cursor-pointer transition-colors"
                      onClick={() => {
                        setView('day');
                        setCurrentDate(day);
                      }}
                    >
                      <div className="text-[12px] font-medium text-zinc-500 capitalize mb-0.5">
                        {format(day, 'EEEE')}
                      </div>
                      <div className="text-[28px] font-bold text-zinc-800 leading-none">
                        {format(day, 'd')}
                      </div>
                    </div>
                  )}
        ...
        <div className="flex-1 overflow-y-auto flex px-4 relative pb-20" ref={scrollContainerRef}>
          ...
          {/* Grid Columns */}
          <div
            className={`flex-1 grid gap-4 ${view === 'day' ? 'grid-cols-1' : 'grid-cols-7'} relative pt-4`}
          >
            ...
            <div className="absolute inset-0 pointer-events-none left-0 right-0 top-4">
              {HOURS.map((hour) => (
                <div
                  key={hour}
                  style={{ height: `${PIXELS_PER_HOUR}px` }}
                  className="border-t border-zinc-100 w-full"
                />
              ))}
            </div>
                  ...
                  {dayEvents.map((event: any, idx: number) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span
                          className={`text-[11px] font-medium opacity-80 ${height < 60 ? 'truncate' : ''}`}
                        >
                          {isAllDay
                            ? 'All Day'
