...
const mockDigestData = {
  focusSuggestion:
    'Your top priority today is signing off on the Q3 roadmap. Marcus and the eng team are blocked.',
  meetings: [
    {
      id: 1,
  ...
  waitingOn: [
    {
      ...
      initials: 'MC',
      name: 'Marcus Chen',
      text: 'Waiting for approval on the revised roadmap.',
      sent: '2D AGO',
    },
    {
      id: 2,
      initials: 'PR',
      name: 'Priya Patel',
      text: 'Needs confirmation on the design review timing.',
      sent: '6H AGO',
    },
    {
      id: 3,
      initials: 'LN',
      name: 'Linear',
      text: 'Issue status update pending from the eng team.',
      sent: '1D AGO',
    },
  ],
  fyi: [
...
const getActionBadgeStyle = (type: string) => {
  switch (type) {
    case 'reply':
      return 'bg-[#111f3d]/50 text-blue-400 border border-blue-500/10';
    case 'decide':
      return 'bg-[#1d1136]/50 text-amber-500 border border-amber-500/10'; // In screenshot "DECIDE" is yellow/amber
    case 'delegate':
      return 'bg-[#0c2323]/50 text-emerald-400 border border-emerald-500/10';
    default:
    ...
    <div className="flex-1 overflow-y-auto bg-[#0a0a0a] text-white">
      <div className="max-w-[1100px] mx-auto py-12 px-10">
        ...
        <div className="flex items-start justify-between mb-8">
          <div>
            <h2 className="text-[10px] font-bold tracking-[0.25em] text-zinc-500 uppercase mb-2">
              Morning Digest
            </h2>
            ...
            <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              <div className="bg-[#1a140a] border border-[#3a2a11] rounded-xl p-5 flex items-start gap-4">
                ...
                <div className="flex-1 text-[14px] text-zinc-400 leading-relaxed">
                  <strong className="text-white font-semibold">Focus suggestion:</strong>{' '}
              ...
              <div className="grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-10">
                ...
                <div className="space-y-10">
                  {/* Today's Meetings */}
                  <section>
                    <h3 className="text-[11px] font-bold tracking-[0.2em] text-zinc-500 uppercase mb-5">
                      Today's Meetings
                    </h3>
                    <div className="space-y-4">
                      {data.meetings.map((meeting) => (
                        <div
                          key={meeting.id}
                          className="relative bg-[#111] border border-[#222] rounded-xl p-5 overflow-hidden"
                        >
                          <div
                            className={`absolute left-0 top-0 bottom-0 w-1 ${meeting.color === 'cyan' ? 'bg-cyan-400' : 'bg-amber-400'}`}
                          />
                          ...
                          <div className="flex justify-between items-start mb-5 pl-2">
                            ...
                            <div className="flex items-center -space-x-1">
                              {meeting.attendees?.map((att, i) => (
                                <div
                                  key={i}
                                  className="w-[22px] h-[22px] rounded-full bg-[#222] border border-[#111] flex items-center justify-center text-[8px] font-bold text-zinc-300"
                                >
                                  {att}
                                </div>
                              ))}
                  ...
                  <section>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      {data.actionItems.map((item) => (
                        <div
                          key={item.id}
                          className="bg-[#111] border border-[#222] rounded-xl p-5 flex flex-col justify-between min-h-[140px]"
                        >
                          <div className="flex justify-between items-start gap-4">
                            ...
                            <div
                              className={`text-[8px] font-bold tracking-[0.15em] uppercase px-2 py-1 rounded shrink-0 mt-0.5 ${getActionBadgeStyle(item.type)}`}
                            >
                              {item.type}
                            </div>
                          </div>
                    ...
                    <div className="space-y-4">
                      {data.waitingOn.map((item) => (
                        ...
                        >
                          ...
                          <p className="text-[13px] text-zinc-400 leading-relaxed pl-11 mb-4">
                            {item.text}
                          </p>
