  ...
  if (error) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center bg-[#F9FAFB] h-full">
        ...
        <button
          onClick={() => mutate()}
          className="px-6 py-2 bg-zinc-900 text-white rounded-xl font-medium shadow-sm hover:bg-zinc-800 transition"
        >
          Try Again
        </button>
      </div>
    ...
    <div className="flex-1 bg-[#F9FAFB] h-full overflow-y-auto font-sans">
      <div className="max-w-6xl mx-auto px-6 py-10 lg:px-12 lg:py-16 space-y-10">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            <p className="text-zinc-400 font-bold tracking-widest text-[11px] uppercase mb-1.5">
              {todayStr}
            </p>
              ...
              <div className="space-y-4">
                {actionItems.map((item: any, idx: number) => {
                  ...
                  return (
                    <div
                      key={idx}
                      className="bg-white border border-zinc-200 rounded-2xl p-5 shadow-sm hover:shadow-md transition-shadow group"
                    >
                      <div className="flex items-start justify-between gap-4 mb-2.5">
                        <span
                          className={`px-2 py-0.5 text-[10px] font-bold uppercase tracking-widest rounded-md ${badgeStyle}`}
                        >
                          {item.type}
                        </span>
                      </div>
                      ...
                      <p className="text-zinc-500 text-[13px]">From: {item.from}</p>
                    </div>
                  );
                })}
              </div>
                ...
                <div className="space-y-5">
                  {meetings.map((m: any, i: number) => (
                    <div key={i} className="border-l-[3px] border-zinc-200 pl-3 py-0.5">
                      <h4 className="text-[14px] font-medium text-zinc-900 leading-tight mb-0.5">
                        {m.title}
                      </h4>
                      {m.time && (
                        <p className="text-[12px] text-zinc-500 mb-1.5">
                          {m.time} {m.attendees?.length > 0 && `• ${m.attendees.join(', ')}`}
                        </p>
                      )}
                      {m.notes && m.notes.length > 0 && (
                        <ul className="text-[13px] text-zinc-600 leading-relaxed list-disc list-inside">
                          {m.notes.map((n: string, idx: number) => (
                            <li key={idx}>{n}</li>
                          ))}
                        </ul>
                      )}
