...
type CorsairUserConnectionUpdate = {
  corsairUserId: string;
  ...
  try {
    ...
    if (!code || !state) {
      return NextResponse.json(
        { success: false, error: { code: 'BAD_REQUEST', message: 'Missing callback parameters' } },
        { status: 400 }
      );
    }
    ...
    const { plugin: integration, tenantId: userId } = await processOAuthCallback(corsair, {
      code,
      state,
      redirectUri,
    });
    ...
    const updateData: CorsairUserConnectionUpdate = {
      corsairUserId,
    };
    ...
    return NextResponse.json(
      {
        success: false,
        error: { code: 'INTERNAL_ERROR', message: getErrorMessage(error, 'Callback failed') },
      },
      { status: 500 }
    );
  }
